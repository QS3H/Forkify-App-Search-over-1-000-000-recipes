...
const addKeyboardSupport = function() {
    document.addEventListener('keydown', function(e) {
        ...
        if (e.key === 'Escape') {
            ...
            if (!modal.classList.contains('hidden')) (0, _addRecipeViewJsDefault.default).toggleWindow();
        }
        // Enter key to submit search
        if (e.key === 'Enter' && document.activeElement.classList.contains('search__field')) document.querySelector('.search').dispatchEvent(new Event('submit'));
    });
};
const init = function() {
...
const loadRecipe = async function(id) {
    try {
        const data = await retryRequest(()=>(0, _helpersJs.AJAX)(`${(0, _configJs.API_URL)}${id}?key=${(0, _configJs.KEY)}`));
        ...
        throw new Error(err.message.includes('404') ? 'Recipe not found. Please check the URL and try again.' : 'Failed to load recipe. Please check your internet connection and try again.');
    }
};
const loadSearchResults = async function(query) {
    try {
        state.search.query = query;
        const data = await retryRequest(()=>(0, _helpersJs.AJAX)(`${(0, _configJs.API_URL)}?search=${query}&key=${(0, _configJs.KEY)}`));
        console.log(data);
        ...
        throw new Error(err.message.includes('404') ? 'No recipes found for your search. Please try different keywords.' : 'Search failed. Please check your internet connection and try again.');
    }
};
...
parcelHelpers.export(exports, "MAX_RECIPES_PER_PAGE", ()=>MAX_RECIPES_PER_PAGE);
parcelHelpers.export(exports, "CACHE_DURATION", ()=>CACHE_DURATION);
const getApiKey = ()=>{
    // In production, this should come from environment variables
    // For development, we'll use a fallback
    return '827cd979-2ddc-49f4-8aec-b5616648fca5';
};
...
const DEBOUNCE_DELAY = 500; // ms for search debouncing
const MAX_RECIPES_PER_PAGE = 50;
...
const getCachedData = function(key) {
    ...
    if (cached && Date.now() - cached.timestamp < (0, _configJs.CACHE_DURATION)) return cached.data;
    return null;
};
...
const AJAX = async function(url, uploadData) {
    ...
    if (!uploadData) setCachedData(url, data);
    return data;
};
...
class SearchView {
    ...
    addHandlerSearch(handler) {
        ...
        const debouncedHandler = (0, _helpersJs.debounce)(handler, (0, _configJs.DEBOUNCE_DELAY));
        this._parentEl.querySelector('.search__field').addEventListener('input', debouncedHandler);
    }
}
exports.default = new SearchView();

},{"@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT","../helpers.js":"7nL9P","../config.js":"2hPh4"}],"kBQ4r":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _viewJs = require("./View.js");
var _viewJsDefault = parcelHelpers.interopDefault(_viewJs);
...
},{"./View.js":"jSw21","url:../../img/icons.svg":"fd0vu","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"f6ot0":[function(require,module,exports,__globalThis) {
/**
